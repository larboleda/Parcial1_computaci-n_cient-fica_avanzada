...
/*Solucion del primer parcial de computación científica. ESte codigo forma triangulos a partir de la busque de los dos vecimos mas cercanos. Calcula cuantos tringulos formaria una particula en caso de no haber mas restricciones para examinar cuantos triangulos se forman en funcion de la distancia al centro de la distribucion.
Ademas calcula la densidad superficial de masa de la distribucion a partir de la creacion de anillos concentricos en los que se distribuyen igual numero de particulas (por lo tando lo anillos tendran un ancho variable). Se Calcula masa total encerrada a un radio R usando el metodo de cimpson compuesto. 
Usando una interpolacion de spline y el metodo de bisección se encuentra el radio para el cual la densidad superficial a caido un factor de e, es decir, densidad = (densidad_central / e).

El codigo debe recibir el archivo con las coordenadas, el numero de particulas total y el numero de particulas que queremos poner en cada anillo. */
